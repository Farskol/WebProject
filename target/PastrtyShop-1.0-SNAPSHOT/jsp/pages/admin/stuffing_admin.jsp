...
<c:set var="currentPage" scope="session" value="jsp/pages/admin/stuffing_admin.jsp"/>
<c:choose>
...
<fmt:bundle basename="context.language">
    ...
    <fmt:message key="stuffingAdmin.editStuffing" var="editStuffing"/>
    <fmt:message key="stuffingAdmin.deleteStuffing" var="deleteStuffing"/>
</fmt:bundle>
<html>
<head>
    <title>Title</title>
...
<nav class="navbar navbar-expand-lg bg-success">
    <div class="container">
        ...
        <div class="d-flex">
            <button type="submit" class="btn btn-success" onclick="window.location.href='${pageContext.request.contextPath}/jsp/pages/admin/stuffing_create.jsp'">${addStuffing}</button>
        </div>
        <div class="d-flex">
...
<div class="container d-flex flex-wrap justify-content-center">
    <c:forEach var="stuff" items="${stuffing_list}">
        <div class="card m-3 col-sm-4" style="width: 16rem;">
            <div class="card-body">
                <form name="addDessert" method="POST" action="${pageContext.request.contextPath}/controller">
                    <input type="hidden" name="command" value="edit_stuffing">
                    <input type="hidden" name="stuffing_id" value="${stuff.stuffingId}">
                    <h5 class="card-title">${stuff.stuffing}</h5>
                    <p class="card-text">${stuff.description}</p>
                    <button type="submit" class="btn btn-success">${editStuffing}</button>
                </form>

                <form name="addDessert" method="POST" action="${pageContext.request.contextPath}/controller">
                    <input type="hidden" name="command" value="delete_stuffing">
                    <input type="hidden" name="stuffing_id" value="${stuff.stuffingId}">
                    <button type="submit" class="btn btn-success">${deleteStuffing}</button>
                </form>
            </div>
        </div>
    </c:forEach>
